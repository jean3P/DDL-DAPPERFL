# src/datasets/digits.py

import torchvision.transforms as transforms
from utils.conf import data_path
from PIL import Image
from .utils.federated_dataset import FederatedDataset, partition_digits_domain_skew_loaders
...
class FedLeaDigits(FederatedDataset):
        ...
        for _, domain in enumerate(using_list):
            if domain == 'syn':
                ...
                if domain == 'mnist':
                    ...
                    train_dataset = MyDigits(data_path()+'/USPS', train=True,
                                             download=True, transform=sin_chan_nor_transform, data_name=domain)
                else:
                    # train_dataset = MyDigits(data_path(), train=True,
                    #                          download=True, transform=nor_transform, data_name=domain)
                    train_dataset = MyDigits(data_path()+'/SVHN', train=True,
                                             download=True, transform=nor_transform, data_name=domain)
        ...
        for _, domain in enumerate(self.DOMAINS_LIST):
            if domain == 'syn':
                ...
                if domain == 'mnist':
                    ...
                    test_dataset = MyDigits(data_path()+'/USPS', train=False,
                                            download=True, transform=sin_chan_test_transform, data_name=domain)
                else:
                    # test_dataset = MyDigits(data_path(), train=False,
                    #                         download=True, transform=test_transform, data_name=domain)
                    test_dataset = MyDigits(data_path()+'/SVHN', train=False,
                                            download=True, transform=test_transform, data_name=domain)
    ...
    def get_backbone(parti_num, names_list):
        nets_dict = {'resnet10': resnet10, 'res10': resnet10, 'resnet12': resnet12,
                     'res18': resnet18, 'resnet18': resnet18, 'efficient': EfficientNetB0, 'mobilnet': MobileNetV2}
        nets_list = []
        if names_list == None:
