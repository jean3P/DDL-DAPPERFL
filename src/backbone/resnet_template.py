# src/backbone/resnet_template.py
...
class ResNet_WD(nn.Module):
    ...
    def __init__(self, block, num_blocks, step_size_2d_list, p_drop, learnable_step=True, num_classes=10,
        ...
        self.layer1 = self._make_layer(block, up(64 * p_drop), num_blocks[0], step_size_2d_list[0],
                                       learnable_step=learnable_step, stride=1, base_width=width_per_group)
        self.layer2 = self._make_layer(block, up(128 * p_drop), num_blocks[1], step_size_2d_list[1],
                                       learnable_step=learnable_step, stride=2, base_width=width_per_group)
        self.layer3 = self._make_layer(block, up(256 * p_drop), num_blocks[2], step_size_2d_list[2],
                                       learnable_step=learnable_step, stride=2, base_width=width_per_group)
        self.layer4 = self._make_layer(block, up(512 * p_drop), num_blocks[3], step_size_2d_list[3],
                                       learnable_step=learnable_step, stride=2, base_width=width_per_group)
    ...
    def _make_layer(self, block, planes, num_blocks, step_size_1d_list, learnable_step, stride, base_width):
        ...
        for stride in strides:
            layers.append(
                block(self.in_planes, planes, step_size_1d_list[i], learnable_step=learnable_step, stride=stride,
...
class ResNet_c_WD(nn.Module):
    ...
    def __init__(self, block, num_blocks, step_size_2d_list, p_drop, learnable_step=True, num_classes=10):
        ...
        self.layer1 = self._make_layer(block, up(16 * p_drop), num_blocks[0], step_size_2d_list[0],
                                       learnable_step=learnable_step, stride=1)
        self.layer2 = self._make_layer(block, up(32 * p_drop), num_blocks[1], step_size_2d_list[1],
                                       learnable_step=learnable_step, stride=2)
        self.layer3 = self._make_layer(block, up(64 * p_drop), num_blocks[2], step_size_2d_list[2],
                                       learnable_step=learnable_step, stride=2)
